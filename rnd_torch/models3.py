...
class base_net(nn.Module):
    def __init__(self, input_shape):
        ...
        self.name = 'base_net'
        self.conv_part = models.resnet34(pretrained=False)
...
class stride_base_net(nn.Module):
    def __init__(self, input_shape):
        super(stride_base_net, self).__init__()
        self.name = 'base_net'
...
class policy_net(nn.Module):
    def __init__(self, input_shape, action_space):
        ...
        self.base_part = base_net(input_shape)
...
class reward_net(nn.Module):
    ...
    def forward(self, state):
        ...
        fast_diff = torch.mean(torch.square(fast-target), dim=-1)
        #slow_diff = torch.mean(torch.square(slow-target), dim=-1)
